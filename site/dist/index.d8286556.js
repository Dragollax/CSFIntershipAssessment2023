...
function searchCockTails() {
    ...
    fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${encodeURIComponent(searchText)}`).then((response)=>response.json()).then((data)=>{
        ...
        else // if there are no drinks then display message
        cocktailContainer.innerHTML = `
...
function getIngredientsStr(drink) {
    let ingredients = [];
    for(let i = 0; i < 15; ++i)if (drink[`strIngredient${i + 1}`]) ingredients.push(drink[`strIngredient${i + 1}`]);
...
//# sourceMappingURL=index.d8286556.js.map
